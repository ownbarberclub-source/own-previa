...
async function run() {
  ...
  const appBarbers = barbers.map(barber => {
    ...
    barberRecords.forEach(rec => {
      ...
      if (rec.category === 'avulso') { avulsoCount++; }
    });

    const actualMinutes = manual ? manual.minutes : subMins;
    return { name: barber.name, appMins: actualMinutes, appAvulso: avulsoCount };
  });
  ...
  cycleRecords.forEach(rec => {
    ...
    if (!resultsMap[key]) {
      resultsMap[key] = { sMins: 0, aCnt: 0 };
    }
    if (rec.category === 'assinatura') { resultsMap[key].sMins += rec.duration_minutes; }
    if (rec.category === 'avulso') { resultsMap[key].aCnt++; }
  });

  const closingBarbers = barbers.map(barber => {
    const key = `${barber.name}-${barber.unit_id}`;
    const data = resultsMap[key] || { sMins: 0, aCnt: 0 };
    const manual = manualMinutes.find(m => m.barber_id === barber.id && m.cycle_id === activeCycle.id);
    const actualMinutes = manual ? manual.minutes : data.sMins;
    return { name: barber.name, closingMins: actualMinutes, closingAvulso: data.aCnt };
  });

  let hasDiff = false;
  appBarbers.forEach(a => {
    const c = closingBarbers.find(b => b.name === a.name);
    if (a.appMins !== c.closingMins || a.appAvulso !== c.closingAvulso) {
      console.log(`Diff for ${a.name}: App(Mins:${a.appMins}, Avulso:${a.appAvulso}) vs Closing(Mins:${c.closingMins}, Avulso:${c.closingAvulso})`);
      hasDiff = true;
    }
  });
